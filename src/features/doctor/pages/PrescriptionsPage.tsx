      ...
      <div className="space-y-6 pb-12 font-sans">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight flex items-center gap-2.5">
              <ClipboardIcon className="h-7 w-7 text-violet-600" />
        ...
        {isLoading ? (
          <div className="py-20 text-center text-slate-400 font-medium space-y-2">
            ...
            <p>Loading digital prescriptions...</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {filteredList?.map((rx) => (
              <Card
                key={rx.id}
                className="relative overflow-hidden rounded-2xl border border-slate-200 shadow-xs hover:shadow-md hover:border-violet-300 transition-all"
              >
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      <CardTitle className="text-base font-bold text-slate-900 flex items-center gap-2">
                        ...
                        {rx.patientName || `Patient #${String(rx.patientId).slice(0, 6)}`}
                      </CardTitle>
                ...
                <CardContent className="p-5 space-y-3.5">
                  ...
                  <div>
                    ...
                    <div className="bg-slate-50 p-3 rounded-xl text-xs font-mono whitespace-pre-wrap border border-slate-200/70 text-slate-800">
                      {rx.medicines}
                    </div>
                  </div>
                  ...
                  <div className="pt-2 border-t border-slate-100 flex justify-end">
                    <Button
                      ...
                      className="text-xs font-bold text-violet-700 border-violet-200 hover:bg-violet-50 rounded-xl gap-1.5 h-8"
                    >
                      <Printer className="h-3.5 w-3.5" /> View / Print Prescription
                    </Button>
                  </div>
                </CardContent>
              </Card>
          ...
          <DialogContent className="sm:max-w-lg rounded-3xl p-6">
            <DialogHeader className="border-b pb-4">
              <div className="flex items-center gap-2">
                <div className="h-8 w-8 rounded-xl bg-violet-600 text-white flex items-center justify-center font-bold">
                  <Stethoscope className="h-4 w-4" />
                </div>
                <div>
                  <DialogTitle className="text-lg font-bold text-slate-900">ArogyaGenie e-Prescription</DialogTitle>
            ...
            {selectedRx && (
              <div className="space-y-4 py-2 text-xs">
                ...
                <div>
                  ...
                  <div className="p-3 rounded-xl bg-slate-50 border border-slate-100 font-mono text-slate-900 whitespace-pre-wrap">
                ...
                {selectedRx.instructions && (
                  <div>
                    ...
                    <p className="text-slate-700">{selectedRx.instructions}</p>
                  </div>
                )}
