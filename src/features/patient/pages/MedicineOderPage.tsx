...
      >
        {/* Header - Navy Blue, Centered Title, Border Completely Hidden, Icons Removed */}
...
              className="absolute left-4 px-3.5 py-1.5 bg-white/10 hover:bg-white/20 text-slate-100 hover:text-white rounded-xl text-xs font-semibold transition-all border-0 outline-none shadow-xs"
            >
              <span>Back</span>
            </button>
          )}
...
            <button 
              onClick={onClose} 
              className="absolute right-4 px-3.5 py-1.5 bg-white/10 hover:bg-white/20 text-slate-100 hover:text-white rounded-xl text-xs font-semibold transition-all border-0 outline-none shadow-xs"
            >
              <span>Cancel</span>
            </button>
          )}
...
                <button onClick={() => startBidding(NEARBY_PHARMACIES, true)} className="w-full bg-white p-4 rounded-2xl border border-slate-200/80 hover:border-indigo-600 hover:shadow-md flex items-center gap-4 transition-all group text-left">
...
                          <span className="flex items-center"><MapPin className="w-3 h-3 mr-0.5 text-slate-400"/>{p.distance} km away</span>
                        </p>
                      </div>
...
                              <span className="flex items-center"><MapPin className="w-3 h-3 mr-1 text-slate-400"/>{q.p.distance} km</span>
...
                                  <span className="text-slate-700 font-medium truncate pr-2">{it.medicine.name} <span className="text-slate-400 text-[10px]">x{it.quantity}</span></span>
                                  <span className="font-bold text-slate-900 shrink-0">₹{(it.price * it.quantity).toFixed(2)} <span className="text-emerald-600 font-bold text-[10px] ml-1 bg-emerald-50 px-1.5 py-0.5 rounded">-{it.disc}%</span></span>
                                </div>
                              ))}
                            </div>
                          </motion.div>
...
              </Button>
            ) : (
              <Button onClick={() => startBidding([NEARBY_PHARMACIES.find(p => p.id === selId)!], false)} disabled={!selId} className="w-full h-11 text-xs rounded-2xl bg-[#13102F] hover:bg-slate-800 font-bold text-white shadow-md transition-all flex items-center justify-center gap-2">
...
      <main className="container mx-auto px-4 sm:px-6 pt-2 sm:pt-3 space-y-4 max-w-7xl">
        
        {/* Action Bar */}
