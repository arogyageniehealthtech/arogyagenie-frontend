// src/pages/patient/LabBookingsPage.tsx
import { useState, useEffect } from 'react';
import { 
  Calendar, Clock, Building2, Search, CalendarDays, 
  AlertCircle, TestTube, User, X, MapPin, Phone, CheckCircle2, ShieldCheck, FileText
} from 'lucide-react';
...
export default function LabBookingsPage() {
  ...
  const [bookings, setBookings] = useState<PatientLabBooking[]>([]);
  const [isLoading, setIsLoading] = useState(true);

  // Centre Info Modal States
  const [centreInfoModalOpen, setCentreInfoModalOpen] = useState(false);
  const [selectedCentreDetails, setSelectedCentreDetails] = useState<any | null>(null);
  ...
  const fetchLabBookings = async () => {
    try {
      setIsLoading(true);
      const rawBookings = await diagnosticApi.getBookedLabs();
      
      const formatted: PatientLabBooking[] = (rawBookings || []).map((b: any) => {
        return {
          id: b.id,
          centreId: b.centreId || 'cen-1',
          centreName: b.centreName || 'Aarogyagenie Diagnostics',
          testName: b.testName || 'Complete Blood Count (CBC)',
          collectionMethod: b.collectionMethod || 'centre',
          date: b.date || 'Today',
          time: b.time || '10:00 AM',
          status: b.status || 'CONFIRMED',
          patientName: b.patientDetails?.name || 'Patient',
          rate: b.rate || 450,
          imageUrl: 'https://images.unsplash.com/photo-1579165466741-7f35e4755660?auto=format&fit=crop&q=80&w=150&h=150',
          raw: b,
        };
      });
      ...
      console.warn('Could not load lab bookings from storage:', err);
      setBookings([
        {
          id: 'lab_sample_1',
          centreId: 'cen-1',
          centreName: 'Apollo Diagnostics',
  ...
  const handleCentreInfoClick = async (booking: PatientLabBooking) => {
    try {
      const details = await diagnosticApi.getCentreById(booking.centreId);
      setSelectedCentreDetails(details || {
        name: booking.centreName,
        phone: '+91 98765 43210',
        address: 'Main Road, Khardaha, West Bengal',
        rating: 4.8,
        reviewCount: 142,
  ...
  const handleConfirmCancel = async () => {
    ...
    try {
      const stored = JSON.parse(localStorage.getItem('mock_lab_bookings') || '[]');
      const updated = stored.map((item: any) => 
        item.id === targetCancelId ? { ...item, status: 'CANCELLED', cancelReason } : item
      );
      localStorage.setItem('mock_lab_bookings', JSON.stringify(updated));

      toast({
        title: 'Lab Booking Cancelled',
        description: 'Your diagnostic lab appointment has been cancelled successfully.',
      });
      ...
      toast({
        title: 'Cancel Failed',
        description: 'Failed to cancel lab test reservation',
        variant: 'destructive',
      });
    }
  };
  ...
  const isCompleted = (s: string) => ['COMPLETED', 'REPORT_READY', 'completed', 'report_ready'].includes(s);
  ...
  const filteredBookings = bookings.filter(b => {
    ...
    const matchesSearch = b.centreName.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-5 pt-4 sm:pt-6 pb-16 px-3 sm:px-6 font-sans">
      ...
      <div className="space-y-3.5">
        {isLoading ? (
          <div className="py-20 text-center text-slate-400 font-medium space-y-2">
            ...
            <p className="text-xs">Loading lab test bookings...</p>
          </div>
          ...
          <div className="bg-white border border-slate-200/80 border-dashed rounded-2xl p-12 text-center shadow-sm flex flex-col items-center">
            ...
            <h3 className="font-bold text-slate-900 text-sm">No lab bookings found</h3>
            <p className="text-xs font-medium text-slate-500 mt-0.5 max-w-xs">You don't have any diagnostic orders matching this category or search.</p>
          </div>
        ) : (
                ...
                <div className="flex items-start gap-3.5">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-xs font-bold text-[#5B21B6] uppercase tracking-wide flex items-center gap-1">
                      <TestTube className="w-3.5 h-3.5" /> {booking.testName} ({booking.collectionMethod})
                    </p>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-3.5 pt-3 sm:pt-0 border-t sm:border-t-0 border-slate-100">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    {upcoming && (
                      <>
                        <button 
                          onClick={() => handleCentreInfoClick(booking)}
                          className="px-3.5 py-2 bg-[#5B21B6] hover:bg-[#4c1d95] text-white rounded-xl font-bold text-xs shadow-md transition-all flex items-center gap-1.5 cursor-pointer"
                        >
                          <Building2 className="w-3.5 h-3.5" /> Centre Info
                        </button>
                        <button 
                          onClick={() => handleCancelClick(booking.id)}
                          className="px-2.5 py-2 bg-rose-50 hover:bg-rose-100 text-rose-600 border border-rose-200 rounded-xl font-bold text-xs transition-colors cursor-pointer"
                    ...
                    {completed && (
                      <button 
                        onClick={() => navigate(ROUTES.PATIENT.LAB_REPORTS)}
                        className="px-3.5 py-2 bg-white hover:bg-slate-50 text-slate-700 border border-slate-200 rounded-xl font-bold text-xs transition-colors shadow-xs cursor-pointer flex items-center gap-1"
                      >
                        <FileText className="w-3.5 h-3.5 text-[#5B21B6]" /> View Report
                      </button>
                    )}

                    {cancelled && (
                      <button 
                        onClick={() => navigate(ROUTES.PATIENT.LAB)}
                        className="px-3.5 py-2 bg-[#5B21B6]/10 hover:bg-[#5B21B6]/20 text-[#5B21B6] rounded-xl font-bold text-xs transition-colors cursor-pointer"
                      >
                        Book Again
                      </button>
                    )}
      ...
      {/* Centre Info Detail Modal */}
      <Dialog open={centreInfoModalOpen} onOpenChange={setCentreInfoModalOpen}>
        <DialogContent className="sm:max-w-lg rounded-3xl p-0 overflow-hidden font-sans">
            ...
            <div className="flex items-center gap-3.5">
              ...
              <div className="min-w-0">
                <h3 className="font-extrabold text-base sm:text-lg tracking-tight truncate">
                  {selectedCentreDetails?.name || 'Diagnostic Centre'}
                </h3>
                <p className="text-xs text-slate-300 mt-0.5 flex items-center gap-1">
                  <ShieldCheck className="w-3.5 h-3.5 text-emerald-400 shrink-0" /> Verified Diagnostic Partner
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="p-5 space-y-4 bg-white text-xs">
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100 space-y-1">
                <p className="text-[10px] font-bold uppercase text-slate-400">Rating & Reviews</p>
                <p className="font-extrabold text-slate-900 flex items-center gap-1 text-sm">
                  ⭐ {selectedCentreDetails?.rating || '4.8'} <span className="text-[11px] font-medium text-slate-500">({selectedCentreDetails?.reviewCount || 120} reviews)</span>
                </p>
              </div>

              <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100 space-y-1">
                <p className="text-[10px] font-bold uppercase text-slate-400">Operating Hours</p>
                <p className="font-extrabold text-slate-900 text-xs flex items-center gap-1 pt-0.5">
                  <Clock className="w-3.5 h-3.5 text-[#5B21B6]" /> {selectedCentreDetails?.timings || '07:00 AM - 09:00 PM'}
                </p>
              </div>
            </div>

            <div className="space-y-1.5">
              <p className="text-[10px] font-bold uppercase text-slate-400">Address & Location</p>
              <p className="font-medium text-slate-700 flex items-start gap-1.5 bg-slate-50 p-3 rounded-xl border border-slate-100">
                <MapPin className="w-4 h-4 text-[#5B21B6] shrink-0 mt-0.5" />
                <span>{selectedCentreDetails?.address?.line1 || selectedCentreDetails?.address || 'Main Road, Khardaha, West Bengal'}</span>
              </p>
            </div>

            <div className="space-y-1.5">
              <p className="text-[10px] font-bold uppercase text-slate-400">Contact Number</p>
              <p className="font-medium text-slate-700 flex items-center gap-1.5 bg-slate-50 p-3 rounded-xl border border-slate-100">
                <Phone className="w-4 h-4 text-[#5B21B6] shrink-0" />
                <span>{selectedCentreDetails?.phone || '+91 98765 43210'}</span>
              </p>
            </div>

            <div className="space-y-2 pt-1">
              <p className="text-[10px] font-bold uppercase text-slate-400">Accreditations & Highlights</p>
              <div className="flex flex-wrap gap-1.5">
                {(selectedCentreDetails?.accreditations || ['NABL Accredited', 'CAP Certified', 'Home Collection Available']).map((badge: string, idx: number) => (
                  <span key={idx} className="bg-indigo-50 border border-indigo-100 text-indigo-700 px-2.5 py-1 rounded-lg text-[11px] font-bold flex items-center gap-1">
                    <CheckCircle2 className="w-3 h-3 text-indigo-600" /> {badge}
                  </span>
                ))}
              </div>
            </div>
          ...
          <DialogFooter className="px-5 py-3 bg-slate-50 border-t border-slate-100">
            <Button onClick={() => setCentreInfoModalOpen(false)} className="w-full bg-[#0F172A] hover:bg-slate-800 text-white rounded-xl text-xs font-bold py-2.5">
              Close Details
            </Button>
          </DialogFooter>
        </DialogContent>
        ...
        <DialogContent className="sm:max-w-md rounded-3xl">
          <DialogHeader>
            <DialogTitle className="text-lg font-bold text-slate-900 flex items-center gap-2">
              <AlertCircle className="h-5 w-5 text-rose-500" /> Cancel Lab Reservation
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-3 py-2 text-xs">
            <p className="text-slate-600">
              Are you sure you want to cancel this lab test? Please state a reason:
            </p>
            <Input
              placeholder="e.g. Test no longer required / Rescheduling"
              value={cancelReason}
