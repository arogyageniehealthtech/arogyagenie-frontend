...
export function AdminPendingApplicationsPage() {
  ...
  const handleReject = () => {
    ...
    rejectMutation.mutate(
      { id: selectedApp.id, reason: rejectionReason },
      {
        onSuccess: () => {
          toast({
  ...
  const filteredApps = applications.filter((app: ProviderApplication) => {
  ...
  const getTypeIcon = (type: string) => {
    ...
    if (type === "DIAGNOSTIC_CENTER") return <Building className="h-4 w-4 text-emerald-600" />;
          ...
          <CardContent className="pt-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <div className="flex flex-wrap gap-2">
                ...
                >
                  <Clock className="h-4 w-4 mr-2" /> Pending ({applications.filter((a: ProviderApplication) => a.status === "PENDING").length})
                </Button>
                <Button
                  ...
                  className="rounded-full px-5 text-sm"
                >
                  <Check className="h-4 w-4 mr-2" /> Approved ({applications.filter((a: ProviderApplication) => a.status === "APPROVED").length})
                </Button>
                <Button
                  ...
                  className="rounded-full px-5 text-sm"
                >
                  <X className="h-4 w-4 mr-2" /> Rejected ({applications.filter((a: ProviderApplication) => a.status === "REJECTED").length})
                </Button>
                <Button
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-slate-100 text-sm">
                    {filteredApps.map((app: ProviderApplication) => (
...
export const PendingApplicationPage = AdminPendingApplicationsPage;
