  ...
  return (
    <div className="relative w-full h-full" ref={dropdownRef}>
      ...
      <button
        ...
        className={`w-full h-full min-h-[44px] bg-white/70 border rounded-xl px-2.5 md:px-4 flex items-center justify-between text-xs md:text-sm transition-all focus:outline-none focus:ring-2 focus:ring-[#5B21B6]/20 ${
          isOpen 
      ...
      >
        ...
        <ChevronDown 
          className={`w-3 h-3 md:w-4 md:h-4 text-gray-400 transition-transform duration-200 flex-shrink-0 ${isOpen ? 'rotate-180 text-[#5B21B6]' : ''}`} 
        />
      </button>
              ...
              return (
                <li key={index}>
                  ...
                  >
                    ...
                    {isSelected && (
                      <Check className="w-3 h-3 md:w-4 md:h-4 text-[#5B21B6] flex-shrink-0" />
                    )}
                  </button>
                </li>
              );
